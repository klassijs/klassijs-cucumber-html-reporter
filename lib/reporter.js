...
import { collectJSONS } from './jsonDir.js';
import { emojify } from 'node-emoji';
import searchFileUp from './searchFileUp.js';
...
const generateReport = function (options) {
  ...
  const parseScenarioHooks = function (data) {
    ...
    if (options.storeScreenshots && !fs.existsSync(screenshotsDirectory)) {
      fs.mkdirsSync(screenshotsDirectory);
    }
  ...
  function readFile(fileName) {
    ...
    const resolvedPath = getPath(fileName);
  ...
  fs.writeFileSync(
    ...
    _.template(readFile('index.html'))({
      ...
      piechart:
        options.theme === 'bootstrap' || options.theme === 'hierarchy' ? readFileForRespectiveTemplates('piechart.js') : undefined, guid: guid,
    })
  );
  ...
  console.log(
    '\n' +
      chalk.green.bold(
        emojify(':rocket:') + ' Cucumber HTML report ' + chalk.blue.bold(options.output) + ' generated successfully '
      ) + emojify(':thumbsup:')
  );
};
